...
namespace CRC
{
	...
	__forceinline    unsigned int CRCSecthion(PVOID base)
	{
		...
		for (auto i = 0; i < headers->FileHeader.NumberOfSections; ++i)
		{
			...
			{
				match = crc32(static_cast<char*>(base) + section->VirtualAddress, section->Misc.VirtualSize);
	...
	__forceinline    unsigned int CRCSecthionByName(PVOID base, const char* secthionName)
	{
		unsigned int match = 0;
		...
		{
			auto* section = &sections[i];
			if (ApiWrapper::memcmp(section->Name, secthionName, ApiWrapper::strlen(secthionName)) == 0 && !(sections->Characteristics & IMAGE_SCN_MEM_WRITE))
			{
				match = crc32(static_cast<char*>(base) + section->VirtualAddress, section->Misc.VirtualSize);
